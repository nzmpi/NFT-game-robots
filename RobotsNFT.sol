...
/*
* @title A simple NFTs with OwnableRoles
...
contract RobotsNFT is ERC721, OwnableRoles {
    ...
    function burn(uint256 robotId) external {
        ...
        delete robots[robotId]; // can be removed to keep stats of burned robots, also requires an update in getStats(.)
    }
    ...
    function getStats(uint256 robotId) external view returns (uint8 attack, uint8 defence, uint32 readyTime) {
        require(_exists(robotId), "The robot doesn't exist!"); // remove, if 'delete robots[.]' in burn(.) is removed
